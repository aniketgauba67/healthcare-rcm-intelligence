...
Every curated field carries a classification (docs/project_rules.md §3):
...
Public CMS code files matching the **FY2023 / 2023** claims vintage (docs/project_rules.md §2
...
- Synthetic claims carry **synthetic** provider/facility identifiers
  ...
  simulated crosswalk (docs/project_rules.md §3.4), which is classified `SIMULATED`.
...
presented as a real correspondence (docs/project_rules.md §3.4). All non-`sim_` warehouse
...
present — the provider column is the synthetic `prvdr_num` (docs/project_rules.md §3.4).
...
table they read; all are pre-submission facts by construction (docs/project_rules.md §4):
...
features that read the outcome at all, they do so under the docs/project_rules.md §4.2
...
docs/project_rules.md §4.1 requires a build-failing test whenever a forbidden column enters a
...
in `.gitignore`. docs/project_rules.md §2 locks the hosted demo to a bundled Parquet/DuckDB
...
|---|---|---|---|---|---|
| `vw_claim_enriched` | MIXED | 20,867 | 77 | 54 | yes |
...
| `vw_work_queue_priority` | MIXED | 2,663 | 16 | 12 | yes |
...
1. **The release candidate was built from a clean working tree.** Its
   ...
   `01b1a623b22a5d3295087ace8995025ced0d462d`. The artifact SHA-256 is
   `559022e2fc27461fb874294eca1a6ba39149367afe7b2e49f9b43c2ccc7e8896`.
   Regenerated by the bundle-column `sim_` marker rename; the previously
   QA-accepted artifact was `ab2aa415` / `66456ebf`.
