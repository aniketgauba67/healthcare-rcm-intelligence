...
1. **Coverage.** Every tabular file this project writes must be covered by a
   ...
   The scan is INVERTED, and the first version was not. It used to ask "is every
   file under these three hardcoded roots declared?", which catches a new FILE and
   misses a new DIRECTORY — a whole output tree could appear and rule 1 would stay
   green over it, having never looked. The evidence that it did not self-extend is
   that `dashboard/demo_data` had to be hand-added to `PUBLISHED_ROOTS` when
   app-engineer asked for it; nothing failed while it was missing. So the question
   is now the other way round: every tabular file in the repository that is OURS
   must live under a DECLARED root. Default-deny, so a new output directory fails
   the build the day it appears, which is what this docstring already promised.
   `NON_OUTPUT_TREES` carries the exclusions, each with its reason.
...
3. **Marking (docs/project_rules.md §3.2).** A column whose declared LINEAGE touches a
...
# The four classes docs/project_rules.md §3.1 defines. Nothing else is a classification here:
...
# Trees the inverted scan does NOT treat as our output, and why. This mapping is
# what lets rule 1 be default-deny: a tabular file that is neither excluded here
# nor under a declared `PUBLISHED_ROOTS` entry FAILS, so the next output directory
# announces itself instead of being noticed later.
#
# An exclusion is a claim that we do not AUTHOR the file, and it costs a sentence
# for the same reason `marker_exempt` does. Note what is deliberately absent:
# `mlruns/` (gitignored, in `.gitignore`, does not exist yet). If anyone turns on
# MLflow tracking, rule 1 fires and someone decides whether that tree is published
# — which is the behaviour being bought here, so pre-excluding it would give the
# purchase away.
NON_OUTPUT_TREES: dict[str, str] = {
    ".git": "Git's own object store.",
    ".venv": "Third-party packages. lifelines, statsmodels and numpy ship CSV test "
    "fixtures; none of it is ours and none of it is published.",
    "__pycache__": "Compiled bytecode, regenerated by the interpreter and never authored.",
    ".pytest_cache": "The test runner's own cache, rewritten on every run.",
    ".ruff_cache": "The linter's own cache, rewritten on every run.",
    ".mypy_cache": "The type checker's own cache, rewritten on every run.",
    "node_modules": "Third-party JavaScript packages, none of them ours.",
    ".ipynb_checkpoints": "Jupyter's autosaved copies of notebooks, not an output we publish.",
    # The warehouse's INPUT and STAGING tiers.
    # the simulation output tier to simulation-engineer; §3.6 says raw CMS files are
    # never modified and never committed. Nothing here is read as a report — these
    # are what the loader consumes, not what a person opens — and all four are
    # gitignored by name in `.gitignore`.
    #
    # Listed as the FOUR TIERS rather than as `data/`, on purpose. Excluding the
    # parent would let a genuinely published output escape by being written to a new
    # `data/<something>/`; naming the tiers means such a directory fails, which is
    # the whole point of inverting the scan. If a published extract ever belongs
    # under `data/`, register it in `PUBLISHED_ROOTS` — do not widen this entry.
    "data/raw": "Raw CMS source files (docs/project_rules.md §3.6): never modified, never "
    "committed, an input rather than an output.",
    "data/validated": "Validated staging tier, consumed by the warehouse loader.",
    "data/curated": "Curated staging tier, consumed by the warehouse loader.",
    "data/simulated": "The simulation layer's own output tier, loaded into `sim_` "
    "warehouse tables. Owned by simulation-engineer; the published views over it are "
    "what a reader sees, and those are registered surfaces.",
}
...
_EVALUATION_TABLE = (
    ...
    "slice_payer.csv explicitly (docs/project_rules.md §3.5). FLAGGED FOR REVIEW: `base_rate` in "
...
PUBLISHED_SURFACES: tuple[PublishedSurface, ...] = (
    ...
    PublishedSurface(
        ...
        undeclared_reason="The hosted-demo bundle (docs/project_rules.md §2), and the most exposed "
...
def excluded_because(relative_path: str) -> str | None:
    """The `NON_OUTPUT_TREES` reason covering this path, or None."""
    for prefix, reason in NON_OUTPUT_TREES.items():
        if relative_path == prefix or relative_path.startswith(prefix + "/"):
            return reason
    return None


def our_tabular_files(root: pathlib.Path = REPO_ROOT) -> list[str]:
    """Every tabular file in the repository that this project authors.

    The whole tree, minus `NON_OUTPUT_TREES`. Excluded directories are pruned
    during the walk rather than filtered afterwards, so `.venv` is never
    descended into.
    """
    found: list[str] = []
    for dirpath, dirnames, filenames in os.walk(root):
        relative_dir = pathlib.Path(dirpath).relative_to(root).as_posix()
        prefix = "" if relative_dir == "." else relative_dir + "/"
        dirnames[:] = sorted(d for d in dirnames if excluded_because(prefix + d) is None)
        for name in sorted(filenames):
            if pathlib.PurePosixPath(name).suffix.lower() in TABULAR_SUFFIXES:
                found.append(prefix + name)
    return found


def under_published_root(relative_path: str) -> bool:
    """Whether a path sits inside a declared `PUBLISHED_ROOTS` entry."""
    return any(
        relative_path == root or relative_path.startswith(root + "/") for root in PUBLISHED_ROOTS
    )
...
def assert_schema_is_marked(schema: PublishedSchema) -> None:
    ...
    if offenders:
        raise ProvenanceError(
            ...
            f"{offenders}. docs/project_rules.md §3.2 — the marker survives the rename, so a CSV, a "
...
def assert_every_published_file_is_covered(root: pathlib.Path = REPO_ROOT) -> None:
    ...
    Two questions, in this order, because the second cannot see what the first
    catches: does every tabular file we author live under a DECLARED root, and is
    every file under a declared root covered by a registered surface?
    """
    outside = [path for path in our_tabular_files(root) if not under_published_root(path)]
    if outside:
        directories = sorted({pathlib.PurePosixPath(path).parent.as_posix() for path in outside})
        raise ProvenanceError(
            f"tabular files outside every declared published root: {outside}. The "
            f"directories are {directories}.\n"
            "Rule 1 scans the repository and requires our outputs to live under a declared "
            f"root, so a NEW OUTPUT DIRECTORY fails here the day it appears. Before this was "
            "inverted the scan only looked inside the roots it already knew, which caught a "
            "new file and missed a new tree.\n"
            "Three ways forward, and the right one depends on what the file is:\n"
            f"  * a published output -> add its directory to {__name__}: PUBLISHED_ROOTS and "
            "register the file in PUBLISHED_SURFACES with a schema or a written reason;\n"
            f"  * not ours to publish (third-party, cache, an input tier) -> add it to "
            f"{__name__}: NON_OUTPUT_TREES with the sentence saying why;\n"
            "  * written by a test into the repository instead of tmp_path -> fix the test.\n"
            "Do not widen an existing NON_OUTPUT_TREES entry to a parent directory to make "
            "this pass; that re-opens the hole for everything else under the parent."
        )
