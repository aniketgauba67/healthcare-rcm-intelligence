...
suspiciously good number is a bug (docs/project_rules.md §1).
"""
...
# Providers are the facility slice (docs/project_rules.md §3.4: group on the synthetic
...
def _save_figure(fig, path: pathlib.Path) -> pathlib.Path:  # noqa: ANN001
    """Stamp the synthetic-data banner onto a figure and write it out.

    THE ONLY PLACE IN THIS PROJECT THAT CALLS `savefig`, deliberately. A PNG is
    the most detachable artifact here — it gets dragged into a slide, pasted into
    a chat, screenshotted into a portfolio — and by the time anyone reads it,
    `docs/model_card.md` and its warning are nowhere nearby. docs/project_rules.md §1 has no
    exception for charts, and §6 makes the same demand of every dashboard page.

    The banner used to live inline in `_calibration_plot`, so the calibration
    curve carried it and the SHAP chart — the more quotable of the two — did not.
    Routing both through one writer is what stops that recurring: a new plot gets
    the banner by construction, and a new function that calls `savefig` directly
    to avoid this one fails `tests/leakage/test_plot_provenance_banner.py`, which
    checks structurally (AST) that every figure-writing function also writes a
    banner. Adding a plot without provenance now takes a deliberate act, not an
    oversight.
    """
    import matplotlib.pyplot as plt
...
def _shap_plot(
    ...
    fig.gca().set_xlabel("mean |SHAP value| (average impact on predicted denial risk)", fontsize=9)
    return _save_figure(fig, path)


def write_provenance_readme(
    artifact_dir: pathlib.Path, model: str, make_target: str, description: str
) -> pathlib.Path:
    """Drop a provenance note beside the CSVs a run writes.

    `metrics.json` embeds a `provenance` key; the CSVs next to it embed nothing,
    and a CSV is opened alone far more often than a JSON report is read through.
    Two of these files are the cases docs/project_rules.md names outright: `slice_payer.csv`
    is a five-payer denial-rate comparison and §3.5 requires the simulated-data
    banner on *every* payer-level analysis — there is no payer dimension in the
    real source data at all — while `work_queue_backtest.csv` reads as an
    operational worklist, claim ids and dollar amounts and recommended actions,
    with nothing on the row saying the denial being worked never happened.

    One note per directory rather than a header on every CSV: a header would have
    to survive every `to_csv` call and would corrupt the files for pandas, and a
    ...
    if (artifact_dir / "slice_payer.csv").exists():
        callouts.append(
            ...
            "  invented** (docs/project_rules.md §3.5). Medicare FFS has exactly one payer; the\n"
...
## Read this before quoting any file in this directory

The claims are official CMS **synthetic** Medicare data (SOURCE). The denials,
appeals, payments and workflow events these models are trained and scored on are
**generated by this project's own simulation layer** and are prefixed `sim_`
wherever they appear (docs/project_rules.md §3). **No real patient, provider, payer or
adjudication data exists anywhere in this repository.**
...
Provenance for the training data itself: `docs/provenance_register.md`.
Model behaviour, limitations and every headline figure: `docs/model_card.md`.
"""
    )
    return path
...
def run_model_a(
    ...
    if write:
        # Persist the matrix the run is about to fit on, so the docs/project_rules.md §4.1
    ...
    # --- the comparison docs/project_rules.md §7 actually asks for -----------------------
    comparisons = {
    ...
    threshold_report = {
        ...
        "cost_matrix": {
            **economics,
            ...
            "docs/assumptions.md publishes them, and docs/project_rules.md §4.5 firewalls "
    ...
    report: dict[str, Any] = {
        ...
        "run_stamp": run_stamp(),
        "seed": seed,
        "provenance": "Label and all sim_ features are SIMULATED (docs/project_rules.md §3). "
